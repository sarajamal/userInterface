...
function Delete404(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsProduction/Delete404/' + id, // Use the provided ID parameter
...
function Deletec1404(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsProduction/Deletec1404/' + id, // Use the provided ID parameter
                success: function (data) {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
...
function AddRowcomponentUpdate2404() { //صفحة التعديل . 
    ...
    newRow.innerHTML = `
        ...
        <button type="button" class="btn btn-style5" data-row-index="${newRowNumber}" onclick="DeleteRow91404(this)">حذف</button>
        </td>
            `;
...
function DeleteRow91404(button) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            updateRowIndicesAfterDeletion91404(rowIndex);
        }
    });
}
function updateRowIndicesAfterDeletion91404(deletedIndex) {
...
function DeleteRow99404(button) {
    var rowIndex = parseInt(button.getAttribute("data-row-index"));
...
//صفحة الإضافة 
var newIndex = 0;
function AddRowcomponentnew404() {
    var tableBody = document.querySelector("#tbComponant tbody");
    ...
    newRow.innerHTML = `
        ...
        <td style="text-align:center;">
            ...
            <button type="button" class="btn btn-style5" data-row-index="${newRowNumber-1}" onclick="DeleteRow99404(this)">حذف</button>
        </td>
    `;
...
function AddRowcomponentnew22404() {
    var tableBody = document.querySelector("#tbComponant tbody");
    var newRow = document.createElement("tr");
    ...
    newRow.innerHTML = `
        ...
        <td style="text-align:center;">
            <input type="hidden" name="componontVMList2[${newRowNumber }].ProductionFK" value="${componentFk}" />
            <button type="button" class="btn btn-style5" data-row-index="${newRowNumber }" onclick="DeleteRow99404(this)">حذف</button>
        </td>
    `;
