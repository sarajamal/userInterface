...
function DeleteToolVariety2404(id) { //هذي فقط للعرض البرمجة في controller , هذي للحذف بعد الحفظ في قاعدة البيانات . 
    Swal.fire({
        title: 'تأكيد !!',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsProduction/DeleteToolVariety2404/' + id, // Use the provided ID parameter
...
//الإضافة
function DeleteToolVariety2t1404(id) { //هذي فقط للعرض البرمجة في controller , هذي للحذف بعد الحفظ في قاعدة البيانات . 
    Swal.fire({
        title: 'تأكيد !!',
        text: " تأكد من حفظ الصفوف المضافة في الأدوات قبل الحذف ",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsProduction/DeleteToolVariety2t1404/' + id, // Use the provided ID parameter
                success: function (data) {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
...
function AddRowTool2404(ProductionFK) { //صفحة التعديل 
    ...
    newRow.innerHTML = `
       ...
       <td style="text-align:center;" >
            ...
            <button type="button" class="btn btn-style5" data-row-index="${newRowNumber}" onclick="DeleteRow9404(this)">حذف</button>
        </td>
         `;
...
function DeleteRow9404(button) { 
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            updateRowIndicesAfterDeletion9404(rowIndex);

        }
    });
}

function updateRowIndicesAfterDeletion9404(deletedIndex) {
...
var numeric = 2;
function AddRowToolnew2404(ProductionFK) { //صفحة الإضافة
    var tableBody = document.querySelector("#tblToolVarity2 tbody");
    ...
    newRow.innerHTML = `
       ...
       <td style="text-align:center;" >
            ...
            <button type="button" class="btn btn-style5" data-row-index="${newRowNumber - 1}" onclick="DeleteRow19404(this)">حذف</button> 
        </td>
         `;
...
function AddRowToolnew22404(ProductionFK) { //صفحة الإضافة
    var tableBody = document.querySelector("#tblToolVarity2 tbody");
    ...
    newRow.innerHTML = `
       ...
       <td style="text-align:center;" >
            ...
            <button type="button" class="btn btn-style5" data-row-index="${newRowNumber}" onclick="DeleteRow19404(this)">حذف</button> 
        </td>
         `;
...
//صفحة الإضافة 
function DeleteRow19404(button) { // AJAX قبل تحفظ في قاعدة البيانات ماتحتاج controller
    var rowIndex = parseInt(button.getAttribute("data-row-index"));
    Swal.fire({
        title: 'هل أنت متأكد؟',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            updateRowIndicesAfterDeletion19404(rowIndex);

        }
    });
}
function updateRowIndicesAfterDeletion19404(deletedIndex) {
    var tableBody = document.querySelector("#tblToolVarity2 tbody");
    var rows = tableBody.querySelectorAll("tr");
