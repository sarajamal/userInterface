...
function AddnewFoodReady99(BrandFK) {
    
        $.ajax({
            url: '/admin/SectionDetailsFinishProducts/GetAddID',
            type: 'POST',
            data: {
                BrandFK: BrandFK
            },
...
function loadAndShowModal(button) {
    ...
    fetch(url)
        ...
        .then(html => {
            ...
            if (targetModalId === '#CreateFinshFoodsAdmin') {
                $('#CreateFinshFoodsAdmin').modal('show');
            } else if (targetModalId === '#FinishProductIndexAdmin') {
                $('#FinishProductIndexAdmin').modal('show');
            }
        })
