...
function AddnewRowstepsUpdate303(CleaningFK) { //صفحة التعديل

        // Only retrieve lastID from server on the first click
        $.ajax({
            url: '/admin/SectionDetailsClean/GetAddID',
            type: 'POST',
            data: {
    ...
    function addStep303(CleaningFK) {
...
var clickCount = 0;
var lastID = 0; // Initialize lastID globally
function AddnewRowstepsNew303(CleaningFK) {
...
var clickCount = 0;
var lastID = 0; // Initialize lastID globally
function AddnewRowstepsNew330(CleaningFK) { //صفحة التعديل
        ...
        $.ajax({
            ...
            success: function (response) {AddnewRowstepsNew3
                lastID = parseInt(response);
                addStep330(CleaningFK);
            },
            error: function (xhr, status, error) {
                console.error('Error fetching last ID:', error);
            }
    ...
    function addStep330(CleaningFK) {
...
function Deletestep30(id) { // after save in db . 
    Swal.fire({
        title: 'تأكيد !!!!!',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsClean/Deletestep30/' + id,
...
function Deletestep330(id) { // after save in db . 
    Swal.fire({
        title: 'تأكيد !!!!!',
        text: " تأكد أولا من حفظ أي خطوة قمت بإضافتها قبل الحذف  ",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsClean/Deletestep330/' + id,
                success: function (data) {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
