...
function AddnewFoods123(BrandFK) {
    
        // Only retrieve lastID from server on the first click
        $.ajax({
            url: '/admin/SectionDetailsFoods/GetAddID',
            type: 'POST',
            data: {
                BrandFK: BrandFK
            },
...
function loadAndShowModal(button) {
    ...
    var url = `/admin/${controller}/${action}?id=${id}`;
    ...
    fetch(url)
        ...
        .then(html => {
            ...
            if (targetModalId === '#CreateFoodAdmin') {
                $('#CreateFoodAdmin').modal('show');
            } else if (targetModalId === '#UpdateFoodAdmin') {
                $('#UpdateFoodAdmin').modal('show');
            }
        })
...
document.addEventListener('DOMContentLoaded', function () {
    document.body.addEventListener('click', function (event) {
        if (event.target.matches('.add-button-admin, .food-index-button') || event.target.closest('.add-button-admin, .food-index-button')) {
            const button = event.target.matches('.add-button-admin, .food-index-button') ? event.target : event.target.closest('.add-button-admin, .food-index-button');
