...
function DeleteToolVariety00(id) { //هذي فقط للعرض البرمجة في controller , هذي للحذف بعد الحفظ في قاعدة البيانات . 
    Swal.fire({
        title: 'تأكيد !!',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsPreparation/DeleteToolVariety00/' + id, // Use the provided ID parameter
...
//الإضافة 
function DeleteToolVarietyT100(id) { //هذي فقط للعرض البرمجة في controller , هذي للحذف بعد الحفظ في قاعدة البيانات . 
    Swal.fire({
        title: 'تأكيد !!',
        text: "تأكد من حفظ الصفوف المضافة في الادوات قبل الحذف ",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsPreparation/DeleteToolVarietyT100/' + id, // Use the provided ID parameter
                success: function (data) {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
...
function AddRowTool00(PreparationsFK) { //صفحة التعديل 
    ...
    newRow.innerHTML = `
       ...
       <td style="text-align:center;" >
            ...
            <button type="button" class="btn btn-style5" data-row-index="${newRowNumber - 1}" onclick="DeleteRow666(this)">حذف</button>
        </td>
         `;
...
var numeric = 2;
function AddRowToolnew00(PreparationsFK) { //صفحة الإضافة
    var tableBody = document.querySelector("#tblToolVarity tbody");
    ...
    newRow.innerHTML = `
       ...
       <td style="text-align:center;" >
            ...
            <button type="button" class="btn btn-style5" data-row-index="${newRowNumber-1}" onclick="DeleteRow6111(this)">حذف</button> 
        </td>
         `;
...
//صفحة الإضافة 
function AddRowToolnew2222(PreparationsFK) { //صفحة الإضافة
    var tableBody = document.querySelector("#tblToolVarity tbody");
    ...
    newRow.innerHTML = `
        ...
        <td><input name="ToolsVarityVMList[${newRowNumber}].PrepTools" class="form-control"  placeholder="الأدات المستخدمة" /></td>
       <td style="text-align:center;" >
            <input type="hidden" name="ToolsVarityVMList[${newRowNumber}].PreparationsFK" value="${PreparationsFK}"  />
              
            <button type="button" class="btn btn-style5" data-row-index="${newRowNumber}" onclick="DeleteRow6111(this)">حذف</button> 
        </td>
         `;

    tableBody.appendChild(newRow);
    // Increment the numeric counter when a new row is added
    newRowNumber ++; // Make sure numeric is now set to the next number}
}

//صفحة الإضافة زر الحذف 
function DeleteRow6111(button) { // AJAX قبل تحفظ في قاعدة البيانات ماتحتاج controller
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            updateRowIndicesAfterDeletion6111(rowIndex);

        }
    });
}
function updateRowIndicesAfterDeletion6111(deletedIndex) {
...
function DeleteRow666(button) { // AJAX قبل تحفظ في قاعدة البيانات ماتحتاج controller صفحة التعديل
    var rowIndex = parseInt(button.getAttribute("data-row-index"));
    Swal.fire({
        title: 'هل أنت متأكد؟',
        icon: 'warning',
