...
function Delete00(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsPreparation/Delete00/' + id, // Use the provided ID parameter
                /*type: 'DELETE',*/
...
function DeleteCom2(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/SectionDetailsPreparation/DeleteCom2/' + id, // Use the provided ID parameter
                /*type: 'DELETE',*/
                success: function (data) {
                    if (data.success) {
                        Swal.fire({
...
//صفحة التعديل
function AddRowcomponentUpdate00() {
    ...
    newRow.innerHTML = `
        ...
        <button type="button" class="btn btn-style5" data-row-index="${newRowNumber }" onclick="DeleteRow199(this)">حذف</button>
        </td>
            `;
...
function DeleteRow199(button) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            updateRowIndicesAfterDeletion199(rowIndex);
        }
    });
}

function updateRowIndicesAfterDeletion199(deletedIndex) {
...
function DeleteRow122(button) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            row.remove();
            Swal.fire('Deleted!', 'تم الحذف بنجاح!', 'success');
            updateRowIndicesAfterDeletion122(rowIndex);
        }
    });
}

function updateRowIndicesAfterDeletion122(deletedIndex) {
    var tableBody = document.querySelector("#tbComponant tbody");
    var rows = tableBody.querySelectorAll("tr");
...
function AddRowcomponentnew00() {
    ...
    <button type="button" class="btn btn-style5" data-row-index="${newRowNumber-1}" onclick="DeleteRow122(this)">حذف</button>
    </td>
`;
...
function AddRowcomponentnew2244() {
