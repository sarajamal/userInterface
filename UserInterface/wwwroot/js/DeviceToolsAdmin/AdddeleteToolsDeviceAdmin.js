...
function AddnewtoolsDeviceNew77(BrandFK) {
    // Only retrieve lastID from server on the first click
    $.ajax({
        url: '/admin/SectionDetailsDevice_Tools/GetAddID',
        type: 'POST',
        data: {
            BrandFK: BrandFK
        },
...
function loadAndShowModal(button) {
    ...
    fetch(url)
        ...
        .then(html => {
            ...
            if (targetModalId === '#CreateDeviceToolsAdmin') {
                $('#CreateDeviceToolsAdmin').modal('show');
            } else if (targetModalId === '#IndexDeviceToolsAdmin') {
                $('#IndexDeviceToolsAdmin').modal('show');
            }
        })
