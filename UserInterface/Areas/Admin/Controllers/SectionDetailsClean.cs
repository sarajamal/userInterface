...
namespace UserInterface.Areas.Admin.Controllers
{
    ...
    public class SectionDetailsClean : Controller
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IWebHostEnvironment _webHostEnvironment;

        public SectionDetailsClean(IUnitOfWork unitOfWork, IWebHostEnvironment hostEnvironment)
        {
        ...
        //حفظ BrandFK -----------------------------------------------------------
        public IActionResult RedirectToCleanListAdmin(int brandFK)
        {
            TempData.Keep("BrandFK");
            HttpContext.Session.SetInt32("BrandFK", brandFK);

            return RedirectToAction("CleanListAdmin");
        }

        public IActionResult CleanListAdmin() //this for display List Of التحضيرات Page1
        {
        ...
        public IActionResult RedirectToUpdateCleanInformationAdmin(int? CleanID, int? brandFk)
        {
            // تخزين البيانات في الجلسة
            HttpContext.Session.SetInt32("ID", CleanID ?? 0);
            HttpContext.Session.SetInt32("BrandFK", brandFk ?? 0);

            TempData.Keep("BrandFK");
            return RedirectToAction("UpdateCleanInformationAdmin");
        }
        public IActionResult UpdateCleanInformationAdmin() // After Enter تعديل Display التحضيرات والمكونات...
        {
            // استرجاع البيانات من الجلسة
            int? brandFK = HttpContext.Session.GetInt32("BrandFK");
            int? CleanID = HttpContext.Session.GetInt32("ID");
        ...
        public IActionResult UpdateCleanInformationAdmin(LoginTredMarktViewModel cleanVM)

        {
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("RedirectToUpdateCleanInformationAdmin", new { CleanID = cleanVM.CleanViewModel.CleaningID, brandFK = cleanVM.CleanViewModel.BrandFK });
            }
        ...
        public IActionResult RedirectToCreateCleanInformationAdmin(int? CleanID, int? brandFk)
        {
            // تخزين البيانات في الجلسة
            HttpContext.Session.SetInt32("ID", CleanID ?? 0);
            HttpContext.Session.SetInt32("BrandFK", brandFk ?? 0);
            ...
            TempData.Keep("BrandFK");
            return RedirectToAction("CreateCleanInformationAdmin");
        }
        //صفحة الإضافة   
        public IActionResult CreateCleanInformationAdmin() // After Enter تعديل Display التحضيرات والمكونات...
        {
            // استرجاع البيانات من الجلسة
            int? brandFk = HttpContext.Session.GetInt32("BrandFK");
            int? CleanID = HttpContext.Session.GetInt32("ID");
        ...
        //POST صفحة الاضافة 
        [HttpPost]
        public IActionResult CreateCleanInformationAdmin(LoginTredMarktViewModel clean, int selectCleaning) // After Enter تعديل Display التحضيرات والمكونات...
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (clean.CleanViewModel.CleaningID == 0)  // if Add 
                {
                    ...
                    else
                    {
                        ...
                        double? OldOrder = getIdOrder.CleaningOrder; // Default to 0.0f if Order is null
            ...
            return RedirectToAction("RedirectToCreateCleanInformationAdmin", new { CleanID = clean.CleanViewModel.CleaningID, brandFK = clean.tredMaeketCleanVM.BrandID });
        }
        //زر الحذف في صفحة قائمة التنظيف -------------------------------------------
        #region
        //[HttpDelete]
        public IActionResult DeleteCleanPost505(int? id)
        {
            var deleteSteps = _unitOfWork.StepsCleanRepository3.GetAll().Where(u => u.CleaningFK == id).ToList();
            if (deleteSteps != null)
            {
                for (int i = 0; i < deleteSteps.Count; i++)
                {
                    ...
                    {
                        string imagePath = Path.Combine(_webHostEnvironment.WebRootPath, "IMAGES", IDStep, delet.CleaStepsImage);
            ...
            _unitOfWork.Save();
            return Json(new { success = true, redirectToUrl = Url.Action("RedirectToCleanListAdmin", new { BrandFK = FKBrandToRedyrect1 }) }); //أحتاج يرجع لنفس صفحة التعديل 
        ...
        #endregion
        // --------------------------------------صفحة إضافة الخطوات ------------------------------------------------
        public IActionResult RedirectToCreateCleanStepsAdmin(int? CleanID, int? brandFk)
        {
            // تخزين البيانات في الجلسة
            HttpContext.Session.SetInt32("ID", CleanID ?? 0);
            HttpContext.Session.SetInt32("BrandFK", brandFk ?? 0);
            TempData.Keep("BrandFK");
            return RedirectToAction("CreateCleanStepAdmin");
        }
        public IActionResult CreateCleanStepAdmin() // After Enter تعديل Display التحضيرات والمكونات...
        {
            // استرجاع البيانات من الجلسة
            int? brandFk = HttpContext.Session.GetInt32("BrandFK");
            int? CleanID = HttpContext.Session.GetInt32("ID");
        ...
        [HttpPost]
        public async Task<IActionResult> CreateCleanStepAdmin(LoginTredMarktViewModel cleanVM) // should insert name in Upsert view
        {
            if (ModelState.IsValid)
            {
                int stepsID = cleanVM.CleanViewModel.CleaningID;
                ...
                TempData["success"] = "تم إضافة التنظيف بشكل ناجح";
                return RedirectToAction("RedirectToCreateCleanStepsAdmin", new { CleanID = cleanVM.CleanViewModel.CleaningID, brandFK = cleanVM.CleanViewModel.BrandFK });
            }
            else
            {
                return View(cleanVM);
        ...
        #region API CALLS
        //[HttpDelete]
        public IActionResult Deletestep330(int? id)
        {
            ...
            for (int i = 0; i < subsequentSteps.Count; i++)
            {
                ...
                if (step.CleaStepsID > id)
                {
                    ...
                    _unitOfWork.StepsCleanRepository3.Update(getOld);
                }
            }
            _unitOfWork.Save();

            return Json(new { success = true, redirectToUrl = Url.Action("RedirectToCreateCleanStepsAdmin", new { CleanID = CleaningFK, brandFK = brandfk }) });
        }
        #endregion
        //======================================================================================
        //---------------------------------------------صفحة تعديل الخطوات----------------------------------------------------
        public IActionResult RedirectToUpdateCleanStepAdmin(int? CleanID, int? brandFk)
        {
            // تخزين البيانات في الجلسة
            HttpContext.Session.SetInt32("ID", CleanID ?? 0);
            HttpContext.Session.SetInt32("BrandFK", brandFk ?? 0);
            TempData.Keep("BrandFK");
            return RedirectToAction("UpdateCleanStepAdmin");
        }
        public IActionResult UpdateCleanStepAdmin() // After Enter تعديل Display التحضيرات والمكونات...
        {
            // استرجاع البيانات من الجلسة
            int? brandFK = HttpContext.Session.GetInt32("BrandFK");
            int? CleanID = HttpContext.Session.GetInt32("ID");
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateCleanStepAdmin(LoginTredMarktViewModel cleanVM) // should insert name in Upsert view
        {
            if (ModelState.IsValid)
            {
                int stepsID = cleanVM.CleanViewModel.CleaningID;
                ...
                return RedirectToAction("RedirectToUpdateCleanStepAdmin", new { CleanID = cleanVM.CleanViewModel.CleaningID, brandFK = cleanVM.CleanViewModel.BrandFK });
            }
            else
            {
                return View(cleanVM);
        ...
        #region API CALLS
        //[HttpDelete]
        public IActionResult Deletestep30(int? id)
        {
            var stepsToDelete = _unitOfWork.StepsCleanRepository3.Get(u => u.CleaStepsID == id);
            ...
            return Json(new { success = true, redirectToUrl = Url.Action("RedirectToUpdateCleanStepAdmin", new { CleanID = CleaningFK, brandFK = brandfk }) });
        }
        #endregion
        //======================================================================================
